...
def main():
    ...
    checked_out_projects_path="s"

    with open(f"{WORK_DIR}/resources/defects4j-mf_copy.json","r") as file:
        data=json.load(file)
    with open(f"{WORK_DIR}/resources/defects4jv1-mf.json","r") as file:
    ...
    for bug_id in bug_ids:
        ...
        if True: # bug_id in ['Math_49']:  #.split('_')[0] in ['Chart','Closure','Math','Time','Lang','Mockito'] and bug_id not in processed_bugs.keys():
            try:
                ...
                with open(f"{WORK_DIR}/resources/defects4jv1-mf.json", "w") as file:
