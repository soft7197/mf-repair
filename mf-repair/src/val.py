...
CHECKOUT_DIR = "/Users/aslan/Desktop/MFRepair_ubuntu/resources/checkout"

def main():
    with open ("/Users/aslan/Desktop/MFRepair_ubuntu/resources/validation_data.json", "r") as file:
        data=json.load(file)
    for bug_id in data:
        if bug_id != "Lang_62":
            continue
            ...
            try:
                if 'non_directly_related_tests' in method:
                    ...
                    for patchset in method['extracted_patches']:
                        ...
                        replace_method_auto(f"{CHECKOUT_DIR}/{bug_id}/{method['path']}", patchset[0], method['buggy_function'])
                        run_tests(f"{CHECKOUT_DIR}/{bug_id}")
                        for mm in data[bug_id]['functions']:
                                if get_method_name(mm['buggy_function'] == patch_name) :
                                    replace_method_auto(f"{CHECKOUT_DIR}/{bug_id}/{mm['path']}", mm['buggy_function'], mm['buggy_function'])
                        ...
                        if set(failing_tests) == set([t for t in all_failing_tests if t not in direct_tests]):
                            plausible.append(patchset)
                    method['plausible']=plausible
            except Exception as error:
                with open("/home/selab/Desktop/MFRepair_ubuntu/resources/error_bugs.txt", "r") as file:
                    text=file.read()
                with open("/home/selab/Desktop/MFRepair_ubuntu/resources/error_bugs.txt", "w") as file:
                    file.write(f"{text}\n{bug_id}-{n}-{error}")
                print(f"{bug_id}-{n}-ERROR")  
                
        with open("/home/selab/Desktop/MFRepair_ubuntu/resources/validated_data.json", "w") as file:
